...
void ponerPalabra(int p[]); ///perimte escribir una palabra sin espacios

void mostrarPalabra(int palabra[], int tam); ///permite mostrar la palabra anteriormente escrita
...
/// devuelve la posición que ocupa un número que se envía como parámetro.
///Devuelve la primera posición que encuentra el número. Si no lo encuentra devuelve -1
...
int valorMaximoVector(int v[], int tam);///devuelve  la posición (primera) que ocupa el máximo en el vector
...
void ponerPalabra(char p[]){
        cin >> p;
}

void mostrarPalabra(char palabra[], int tam){
        int j=0;
       while(palabra[j]!='\0'){
        cout << palabra[j];
        j++;
